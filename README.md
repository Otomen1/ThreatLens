...
| **Phase 7 — Investigation Correlation Engine** 🚧 | Deterministically combines a completed investigation's existing findings into higher-level correlation observations (e.g. "malicious infrastructure with exposed services") before higher-level reasoning consumes them. Only combines evidence, never invents it — no AI, no ML, no scoring. **Phase 7.0 (framework)** is complete — canonical models, a declarative rule model + 12-rule seed set, a single generic evaluator, registry, service, `GET /api/v1/correlation`, and a `/correlation` placeholder, with content-addressed timestamp-independent identity and a CI-gated golden snapshot. See [`docs/architecture/PHASE-7.0-CORRELATION-FRAMEWORK.md`](docs/architecture/PHASE-7.0-CORRELATION-FRAMEWORK.md). Rule expansion (Phase 7.1), Timeline/Graph engines, Case Management, and SOAR are later, unstarted milestones. |

All future phases consume the frozen `InvestigationSummary` — the reasoning core does not change to support them. Exposure Intelligence (Phase 5) and Identity Intelligence (Phase 6) are additionally isolated from Threat Intelligence, Knowledge Intelligence, Reasoning, Detection Engineering, the Detection Knowledge Library, and the Operational Dashboard — no shared models, no shared registry, dependency flows one way into `entities/` only. The Investigation Correlation Engine (Phase 7) is a pure, read-only consumer of the `InvestigationSummary` (like the Detection Engine) — it never mutates it and never invents evidence.
