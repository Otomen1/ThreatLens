...
| **Saved Investigations (Phase 8.0)** | A persistence layer over completed investigations — save, load, update, delete, list, search, and filter (by status/severity/type/tag) at **`/workspace`**. Local file-backed storage, no auth, single-user. Consumes existing `InvestigationSummary`/`DetectionPackage` output verbatim; generates nothing. Distinct from the Investigation Workspace analyst UI above — see [`docs/architecture/PHASE-8.0-INVESTIGATION-WORKSPACE.md`](docs/architecture/PHASE-8.0-INVESTIGATION-WORKSPACE.md) for the naming disambiguation. |
...
### Investigation Workspace — persistence layer (Phase 8.0)
...
|---|---|---|
| Backend tests | **2,496 passing, 1 skipped** | Detection, providers, aggregation, reasoning, AI layer, detection engineering, Sigma + YARA + Suricata + Snort + SIEM generation, the community Detection Knowledge Library, correlation, exposure, identity, the Investigation Workspace, API contracts, health/readiness. |
| Frontend tests | 122 passing (Vitest) | API client behaviour incl. `explain()`, `generateDetections()`, `recommendCommunityDetections()`, the Investigation Workspace client (`saveInvestigation`/`listInvestigations`/`getInvestigation`/`updateInvestigation`/`deleteInvestigation`), detection + knowledge helpers, health checks, and abort handling. |
