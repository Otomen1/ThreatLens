...
| **Case Management (Phase 9.0)** | An operational layer **above** the Workspace platform at **`/cases`**: a `Case` organizes zero or more saved investigations by reference, with status, priority, owner, tags, and append-only notes. It uses PostgreSQL when `THREATLENS_STORAGE_BACKEND=postgres`, with SQLite/file fallbacks for local use. |
...
### Reliability, validation, and backup
...
### Backend — Identity Intelligence

| Variable | Default | Purpose |
|---|---|---|
| `IDENTITY_ENABLED` | `false` | Master switch for email identity lookup. |
| `IDENTITY_CACHE_ENABLED` | `true` | Enables the process-local identity result cache. |
| `IDENTITY_CACHE_TTL` | `3600` | Identity result cache TTL in seconds. |
| `IDENTITY_TIMEOUT` | `10` | Per-lookup timeout in seconds. |
| `IDENTITY_RATE_LIMIT_PER_MINUTE` | *(unset)* | Optional local lookup limit. |
| `HIBP_ENABLED` | `true` | Enables the HIBP provider when Identity Intelligence is enabled. |
...
sent or stored. Email history stays in IndexedDB in the current browser for up
to 90 days and is excluded from server backups.
...
`GET /api/v1/identity` reports framework and provider status. Authenticated
email checks use `POST /api/v1/identity/email/check`; privacy-preserving password
range checks use `GET /api/v1/identity/password-range/{prefix}`. The `/identity`
workspace shows breach history, exposed data categories, prior browser-local
comparisons, provider diagnostics, and password exposure results. All results
are descriptive and never a ThreatLens “safe” or “compromised” verdict.
...
| Detection freeze suite | 184 tests / 140-scenario corpus | The **Detection Engine v1.1** contract — every scenario × nine generators, invariants, validators, and golden regression (`backend/tests/detection/`). |
...
| **Phase 6 — Identity Intelligence** ✅ **v1.0** | Personal email breach exposure through optional HIBP access, privacy-preserving password range checks, one-hour result caching, browser-local history/comparison, provider diagnostics, and Full-investigation integration. Results remain descriptive and never declare an identity safe or compromised. Enterprise directories and sign-in telemetry remain out of scope. |
...
| **Phase 9 — Case Management** ✅ | An operational layer **above** the Workspace platform (frozen at Phase 8.5) — **not a Workspace change.** A `Case` organizes zero or more saved investigations by reference (id only, never a content copy); a Workspace investigation may belong to multiple cases, and linking never mutates it. **Phase 9.0** ships the full framework — models (`CaseStatus` with a validated transition graph, `CasePriority`, append-only `CaseNote`), independent local-file storage, service, and eight REST endpoints (`/api/v1/cases`, including link/unlink and notes) — plus a `/cases` list and `/cases/{id}` detail UI. No AI, no automation, no RBAC. See [`docs/architecture/PHASE-9.0-CASE-MANAGEMENT.md`](docs/architecture/PHASE-9.0-CASE-MANAGEMENT.md). SOAR, detection generation from cases, collaboration, and case-level export are later, unstarted milestones. |
...
The **package/release version** advances with every release and remains separate from engine constants. The reasoning engine remains `1.0`; the Detection Engine is `1.1` after the audited generation-issue and field-mapping contract update. Engine-output changes require deliberate golden regeneration and review.
