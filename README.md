...
| **Saved Investigations (Phase 8.0-8.5)** | A persistence layer over completed investigations — save, load, update, delete, list, search, and filter (by status/severity/type/tag) at **`/workspace`**. Local file-backed storage, no auth, single-user. Consumes existing `InvestigationSummary`/`DetectionPackage` output verbatim; generates nothing. Distinct from the Investigation Workspace analyst UI above — see [`docs/architecture/PHASE-8.0-INVESTIGATION-WORKSPACE.md`](docs/architecture/PHASE-8.0-INVESTIGATION-WORKSPACE.md) for the naming disambiguation. Phase 8.1 adds a read-only, derived **Timeline** — chronological events reconstructed only from evidence that already carries an explicit timestamp; see [`docs/architecture/PHASE-8.1-INVESTIGATION-TIMELINE.md`](docs/architecture/PHASE-8.1-INVESTIGATION-TIMELINE.md). Phase 8.2 adds a read-only, derived **Evidence Graph** — entities and relationships reconstructed only from a finding's explicit `Relationship`s and correlation observations; see [`docs/architecture/PHASE-8.2-EVIDENCE-RELATIONSHIP-GRAPH.md`](docs/architecture/PHASE-8.2-EVIDENCE-RELATIONSHIP-GRAPH.md). Phase 8.3 makes that graph **interactive** — pan/zoom/fit-view, click-to-inspect, search, and filter over the same unchanged API data via React Flow; see [`docs/architecture/PHASE-8.3-INTERACTIVE-EVIDENCE-GRAPH.md`](docs/architecture/PHASE-8.3-INTERACTIVE-EVIDENCE-GRAPH.md). Phase 8.4 adds a deterministic **JSON export** and a print-friendly **analyst report view** — a single composed projection of the saved record plus its Timeline and Evidence Graph, at `/workspace/{id}/report`; see [`docs/architecture/PHASE-8.4-WORKSPACE-EXPORT-AND-INVESTIGATION-REPORTING.md`](docs/architecture/PHASE-8.4-WORKSPACE-EXPORT-AND-INVESTIGATION-REPORTING.md). **Phase 8.5** freezes the Workspace/Timeline/Graph/Report/Export contracts (no behavior change), verifies backward compatibility, and records a performance baseline; see [`docs/architecture/WORKSPACE-PLATFORM.md`](docs/architecture/WORKSPACE-PLATFORM.md). |
| **Case Management (Phase 9.0)** | An operational layer **above** the Workspace platform at **`/cases`**: a `Case` organizes zero or more saved investigations by reference (id only — never a content copy), with analyst-assigned status (validated transition graph), priority, owner, tags, and append-only notes. A Workspace investigation may belong to multiple cases; a case never mutates a linked investigation. Independent local-file storage from Workspace's own. See [`docs/architecture/PHASE-9.0-CASE-MANAGEMENT.md`](docs/architecture/PHASE-9.0-CASE-MANAGEMENT.md). |
| **Intelligence Collections (Phase 9.1)** | Reusable, analyst-curated sets of threat intelligence at **`/collections`** — "Silver Fox Campaign", "APT29 Infrastructure", "Internal Blocklist" — built manually or from a Workspace investigation/Case. A `Collection` references (never copies) zero or more Workspace investigations and Cases, and holds deduplicated `Indicator`s across 18 types (IP, domain, hash, CVE, MITRE ATT&CK id, and more), identity `(type, normalized value)`. Re-adding an indicator merges rather than duplicates. No AI, no enrichment, no automatic extraction — every indicator is exactly what an analyst explicitly provided. Independent local-file storage. See [`docs/architecture/PHASE-9.1-INTELLIGENCE-COLLECTIONS.md`](docs/architecture/PHASE-9.1-INTELLIGENCE-COLLECTIONS.md). |
...
```
   ...
   (Ollama, now)    Engineering                      (+ Collections)
```
...
**Phase 8.4** adds `GET /api/v1/workspace/{id}/export` — a deterministic `InvestigationReport` composing the saved record verbatim with the exact same Timeline and Evidence Graph the sibling routes above return, in one call. This backs two presentations: an **Export JSON** download (also available directly from the workspace detail page) and a dedicated, print-friendly **analyst report view** at **`/workspace/{id}/report`** — findings, recommendations, threat-intelligence attribution, correlation, timeline, and a tabular evidence-graph summary, all fully expanded for readability and the browser's native Print/Save-as-PDF. No AI-generated content, no new analytical output; every value is copied from what the pipeline above already produced. See [`docs/architecture/PHASE-8.4-WORKSPACE-EXPORT-AND-INVESTIGATION-REPORTING.md`](docs/architecture/PHASE-8.4-WORKSPACE-EXPORT-AND-INVESTIGATION-REPORTING.md).
...
|---|---|---|
| Backend tests | **3,125 passing, 1 skipped** | Detection, providers, aggregation, reasoning, AI layer, detection engineering, Sigma + YARA + Suricata + Snort + SIEM generation, the community Detection Knowledge Library, correlation, exposure, identity, the Investigation Workspace + Timeline + Evidence Graph, Case Management, Intelligence Collections, API contracts, health/readiness. |
| Frontend tests | 262 passing (Vitest) | API client behaviour incl. `explain()`, `generateDetections()`, `recommendCommunityDetections()`, the Investigation Workspace client (`saveInvestigation`/`listInvestigations`/`getInvestigation`/`updateInvestigation`/`deleteInvestigation`/`getInvestigationTimeline`/`getInvestigationGraph`), Case Management and Intelligence Collections clients, detection + knowledge helpers, health checks, and abort handling. |
...
| Case Management suite | 159 tests | Models, local-file storage (independent of Workspace's own), service (CRUD + filtering + status-transition validation + linking + notes), API contract, and a no-regression suite proving every pre-Phase-9.0 route/engine-version/CORS behavior is unchanged (`backend/tests/cases/`). |
| Intelligence Collections suite | 197 tests | Models, indicator-value normalization (per-type canonical form, malformed-input handling), local-file storage (independent of Workspace's/Cases' own), service (CRUD + filtering + indicator dedup/merge + Workspace/Case linking), API contract (including a route-ordering regression test for `/collections/search`), and a no-regression suite proving every pre-Phase-9.1 route/engine-version/CORS behavior is unchanged (`backend/tests/intel_collections/`). |
...
| **Phase 7 — Investigation Correlation Engine** 🚧 | Deterministically combines a completed investigation's existing findings into higher-level correlation observations (e.g. "malicious infrastructure with exposed services") before higher-level reasoning consumes them. Only combines evidence, never invents it — no AI, no ML, no scoring. **Phase 7.0 (framework)** shipped canonical models, a declarative rule model, a single generic evaluator, registry, service, `GET /api/v1/correlation`, and a `/correlation` placeholder, with content-addressed timestamp-independent identity and a CI-gated golden snapshot. **Phase 7.1 (rule library)** expanded the seed set from 12 to **70 rules** across 7 domain modules, with zero engine/registry/model-structure/API changes. See [`docs/architecture/PHASE-7.0-CORRELATION-FRAMEWORK.md`](docs/architecture/PHASE-7.0-CORRELATION-FRAMEWORK.md) and [`docs/architecture/PHASE-7.1-CORRELATION-RULE-LIBRARY.md`](docs/architecture/PHASE-7.1-CORRELATION-RULE-LIBRARY.md). `/investigate` integration and SOAR are later, unstarted milestones. |
| **Phase 8 — Investigation Workspace** ✅ | A workflow and persistence layer over the completed analytical pipeline — **not a new intelligence engine.** **Phase 8.0** ships save/load/update/delete/list/filter/search over completed investigations (`/api/v1/workspace`, a `/workspace` list + detail UI, and a **Save to Workspace** button on the per-search analyst UI), backed by local file storage behind a swappable `WorkspaceStorage` interface — no auth, no database, single-user, per the phase brief. Consumes the existing `InvestigationSummary`/`DetectionPackage` (and, once a later phase wires Correlation into `/investigate`, `CorrelationSummary`) verbatim; generates nothing. **Phase 8.1** adds a read-only, derived **Investigation Timeline** (`GET /api/v1/workspace/{id}/timeline`): chronological events reconstructed only from evidence that already carries an explicit, timezone-aware timestamp — never invented, estimated, or causally linked. Content-addressed event ids; duplicate evidence cited by more than one finding collapses into one canonical event. **Phase 8.2** adds a read-only, derived **Evidence Relationship Graph** (`GET /api/v1/workspace/{id}/graph`): nodes and edges reconstructed only from a finding's explicit `Relationship`s and a correlation observation's cited findings/relationships — never an invented entity, relationship, or a connection drawn merely from co-occurrence. Content-addressed node/edge ids; a sibling of Timeline, not a consumer of it. **Phase 8.4** adds a deterministic **JSON export** and print-friendly **analyst report view** (`GET /api/v1/workspace/{id}/export`, `/workspace/{id}/report`) composing the saved record with its Timeline and Evidence Graph — no new analytical output, no AI-generated content. **Phase 8.5** is not a feature phase: it freezes the Workspace/Timeline/Graph/Report/Export contracts documented below, verifies backward compatibility across every realistic saved-record shape, records a performance baseline, and confirms no analytical logic is duplicated across the four subsystems — no behavior change. See [`docs/architecture/PHASE-8.0-INVESTIGATION-WORKSPACE.md`](docs/architecture/PHASE-8.0-INVESTIGATION-WORKSPACE.md) (including the disambiguation from the existing, unrelated "Investigation Workspace" analyst UI), [`docs/architecture/PHASE-8.1-INVESTIGATION-TIMELINE.md`](docs/architecture/PHASE-8.1-INVESTIGATION-TIMELINE.md), [`docs/architecture/PHASE-8.2-EVIDENCE-RELATIONSHIP-GRAPH.md`](docs/architecture/PHASE-8.2-EVIDENCE-RELATIONSHIP-GRAPH.md), [`docs/architecture/PHASE-8.4-WORKSPACE-EXPORT-AND-INVESTIGATION-REPORTING.md`](docs/architecture/PHASE-8.4-WORKSPACE-EXPORT-AND-INVESTIGATION-REPORTING.md), and [`docs/architecture/WORKSPACE-PLATFORM.md`](docs/architecture/WORKSPACE-PLATFORM.md). Analyst Notes, Audit History, RBAC, STIX/TAXII, SOAR, and other enterprise features are later, unstarted milestones. |
| **Phase 9 — Case Management & Intelligence Collections** ✅ | An operational and intelligence-asset layer **above** the Workspace platform (frozen at Phase 8.5) — **not a Workspace change.** A `Case` organizes zero or more saved investigations by reference (id only, never a content copy); a Workspace investigation may belong to multiple cases, and linking never mutates it. **Phase 9.0** ships the full Case Management framework — models (`CaseStatus` with a validated transition graph, `CasePriority`, append-only `CaseNote`), independent local-file storage, service, and eight REST endpoints (`/api/v1/cases`, including link/unlink and notes) — plus a `/cases` list and `/cases/{id}` detail UI. **Phase 9.1** adds **Intelligence Collections** — reusable, analyst-curated sets of threat intelligence (`/api/v1/collections`, a `/collections` list + detail UI) that reference zero or more Workspace investigations and Cases; `Indicator`s across 18 types are deduplicated and merged by `(type, normalized value)`, with zero automatic extraction or enrichment. No AI, no automation, no RBAC. See [`docs/architecture/PHASE-9.0-CASE-MANAGEMENT.md`](docs/architecture/PHASE-9.0-CASE-MANAGEMENT.md) and [`docs/architecture/PHASE-9.1-INTELLIGENCE-COLLECTIONS.md`](docs/architecture/PHASE-9.1-INTELLIGENCE-COLLECTIONS.md). SOAR, detection generation from cases/collections, collaboration, case/collection-level export, and automatic intelligence extraction are later, unstarted milestones. |
