...
| **Detection Engineering** | A pure, deterministic downstream consumer that converts findings into a content-addressed `DetectionPackage` (`POST /api/v1/detections`). Ships **Sigma** (IP/domain/URL/hash → traceable Sigma rules) and **YARA** (file hashes → `hash`-module rules) generators; Suricata/SIEM/EDR generators plug into the same registry in later phases. It never alters the investigation. |
...
|---|---|---|
| Backend tests | **1,259 passing** | Detection, providers, aggregation, reasoning, AI layer, detection engineering, Sigma + YARA generation, API contracts, health/readiness. |
| Frontend tests | 26 passing (Vitest) | API client behaviour incl. `explain()`, `generateDetections()`, detection helpers, health checks, and abort handling. |
