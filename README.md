...
| **Detection Engineering** | A pure, deterministic downstream consumer that converts findings into a content-addressed `DetectionPackage` (`POST /api/v1/detections`). Ships a **Sigma generator** (IPs, domains, URLs, file hashes → minimal, traceable Sigma rules); YARA/SIEM/EDR generators plug into the same registry in later phases. It never alters the investigation. |
...
|---|---|---|
| Backend tests | **1,245 passing** | Detection, providers, aggregation, reasoning, AI layer, detection engineering, Sigma generation, API contracts, health/readiness. |
| Frontend tests | 25 passing (Vitest) | API client behaviour incl. `explain()`, `generateDetections()`, detection helpers, health checks, and abort handling. |
