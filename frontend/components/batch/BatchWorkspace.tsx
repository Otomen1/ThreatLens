...
import { BatchPreviewPanel } from "@/components/batch/BatchPreviewPanel";
...
export function BatchWorkspace({ preview, rows, setRows, running, timestamp, onStart, onRetry, onCancel, onClear }: Props) {
    ...
    for (const { row, index } of selected) {
      ...
      try {
        const record = await saveInvestigation({
          ...
          investigation_snapshot: row.investigation,
        });
  ...
  if (preview.requires_confirmation && rows.length === 0) {
    return <BatchPreviewPanel preview={preview} pending={pending} onRemove={(index) => setPending((current) => current.filter((_, itemIndex) => itemIndex !== index))} onStart={() => onStart(pending)} onCancel={onClear} />;
  }
  ...
  return <section className="w-full max-w-5xl mt-10 space-y-4" aria-label="Batch IOC results">
    ...
    <div className="space-y-2">{pageItems.map(({ row, index }) => <BatchResultRow key={`${row.entity.type}-${row.entity.normalized_value}`} row={row} index={index} expanded={expanded.has(index)} running={running} timestamp={timestamp} onSelect={(selectedValue) => updateSelection([index], selectedValue)} onRetry={() => onRetry([index])} onToggle={() => setExpanded((current) => { const next = new Set(current); if (next.has(index)) next.delete(index); else next.add(index); return next; })} />)}</div>
