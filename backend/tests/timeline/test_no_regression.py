"""Regression tests for Phase 8.1: the Investigation Timeline Framework must
not change any existing engine, route, or response shape — including the
Investigation Workspace routes it shares a router file with.

Mirrors ``tests/workspace/test_no_regression.py``'s approach exactly (same
...
class TestExistingRoutesUnaffected:
    def test_every_pre_phase_8_1_route_still_registered(self) -> None:
    ...
    def test_workspace_operations_unchanged_by_the_new_timeline_route(self) -> None:
        paths = app.openapi()["paths"]
    ...
    def test_timeline_route_is_additive(self) -> None:
        paths = app.openapi()["paths"]
        assert "/api/v1/workspace/{investigation_id}/timeline" in paths
...
class TestExistingEngineVersionsUnchanged:
    ...
    def test_timeline_versions_are_new_but_stable(self) -> None:
