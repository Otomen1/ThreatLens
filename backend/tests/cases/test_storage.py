"""Tests for Case Management's LocalFileStorage (Phase 9.0).

Mirrors ``tests/workspace/test_storage.py`` exactly. Every test uses
pytest's ``tmp_path`` fixture — no test ever touches the developer's real
filesystem or the default ``data/cases`` directory, and every test is fully
offline.
"""
...
from threatlens.cases import CaseNotFoundError, CaseStorageError, LocalFileStorage
...
class TestLocalFileStorage:
    ...
    def test_list_all_returns_every_saved_record(self, tmp_path: Path) -> None:
        storage = LocalFileStorage(tmp_path)
        records = [case(title=f"Case {i}") for i in range(5)]
    ...
    def test_second_instance_over_same_root_sees_prior_data(self, tmp_path: Path) -> None:
        ...
        local-file storage."""
        record = case(title="Survives restart")
