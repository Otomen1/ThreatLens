...
def test_empty_detection_is_invalid() -> None:
    artifact = DetectionArtifact(id="det_test", language=DetectionLanguage.SIGMA, target=DetectionTarget(language=DetectionLanguage.SIGMA), title="test")
...
def test_valid_sigma_shape_is_valid() -> None:
    artifact = DetectionArtifact(id="det_test", language=DetectionLanguage.SIGMA, target=DetectionTarget(language=DetectionLanguage.SIGMA), title="test", content="title: Test\nlogsource: {category: dns}\ndetection: {sel: {query: example.com}, condition: sel}\n")
