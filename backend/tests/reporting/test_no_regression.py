"""Regression tests for Phase 8.4: Workspace Export & Investigation Reporting
must not change any existing engine, route, or response shape — including
the Investigation Workspace, Timeline, and Graph routes it shares a router
file with.

Mirrors ``tests/graph/test_no_regression.py``'s approach exactly.
"""
...
class TestExistingRoutesUnaffected:
    def test_every_pre_phase_8_4_route_still_registered(self) -> None:
    ...
    def test_workspace_timeline_and_graph_operations_unchanged(self) -> None:
        paths = app.openapi()["paths"]
    ...
    def test_export_route_is_additive(self) -> None:
        paths = app.openapi()["paths"]
        assert "/api/v1/workspace/{investigation_id}/export" in paths
...
class TestExistingEngineVersionsUnchanged:
    ...
    def test_detection_engine_version_unchanged(self) -> None:
        assert DETECTION_ENGINE_VERSION == "1.2"
    ...
    def test_reporting_version_is_new_but_stable(self) -> None:
