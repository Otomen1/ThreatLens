"""Cache abstractions for the Exposure Intelligence Framework (interfaces only).
...
here is wired into ``ExposureService`` yet.
"""
...
class ExposureCache(ABC, Generic[_T]):
...
class InMemoryExposureCache(ExposureCache[_T]):
